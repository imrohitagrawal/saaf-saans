...
`saafsaans.fly.dev`) — one 256 MB Fly.io machine in Singapore, scaled to
...
**Third parties in the page.** There are none. The type families are self-hosted as
subsetted woff2 files under `/static/fonts` (regenerated by `scripts/build_fonts.py`;
the Devanagari faces are linked only when Hindi is active), so no request leaves this
origin on any page view and no visitor's IP reaches a font host. The page still declares
`<meta name="referrer" content="strict-origin-when-cross-origin">`, so that if a
cross-origin resource is ever added, the request carries the origin alone and never the
age, condition or activity in the query string. That is asserted by
`test_no_page_can_leak_the_persona_in_a_referer_header`, and
`test_no_page_references_any_third_party_origin` fails if any external origin appears;
the CSP (`style-src 'self'; font-src 'self'`) makes the browser refuse one that slips
through anyway.
...
tests/                  1,756 tests, 47 files at `8dedb94`
