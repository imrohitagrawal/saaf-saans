...
# viewport_bands lived here while the counts came from an Elasticsearch terms
# aggregation. The store is a local SQLite file now and its tests moved with it
# to tests/test_viewport_store.py, which keeps the None-versus-[] contract this
# file cared about. The band/band.keyword retry that two of them covered was an
# artefact of Elasticsearch dynamic mapping and has no successor to move to.
